...
class column_op {
   ...
   * 栏目复制；
   */
  public function copy_column($id, $to_lang, $is_contents = 0) {
    global $_M;
    if(!$id){
      return false;
    }
    ...
    $class1 = $this->in_column($c['id'], 0, $to_lang, $is_contents);   
    ...
    if($class1){
      ...
      foreach($son_class2 as $val2){
        ...
        $val_class2 = $this->in_column($val2['id'], $class1, $to_lang, $is_contents);
        ...
        foreach($val_son_class3 as $val3){
          ...
          $val_class3 = $this->in_column($val3['id'], $val_class2, $to_lang, $is_contents);
   ...
   * 复制栏目下内容；
   */
  public function copy_content($column_id, $class1_module, $classnow_module, $toclass1, $toclass2, $toclass3, $to_lang,$is_contents = 0, $paras) {
    ...
    if($classnow_module >=2 && $classnow_module <=5){
      ...
      if($is_contents == 1){
        load::mod_class("{$name}/{$name}_op", 'new')->list_copy($column_id, $toclass1, $toclass2, $toclass3, $to_lang, $paras);
      }
   ...
   * 插入栏目；
   */
  public function in_column($id, $bigclass, $tolang, $is_contents) {
    $infos = load::mod_class('column/column_database', 'new')->get_column_by_id($id);
...
# This program is an open source system, commercial use, please consciously to purchase commercial license.; # Copyright (C) MetInfo Co., Ltd. (http://#). All rights reserved.
?>
