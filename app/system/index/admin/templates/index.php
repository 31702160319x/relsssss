...
<link rel="stylesheet" href="{$_M['url']['ui']}css/box.css?{$jsrand}" />
...
<div class="metcms_cont v52fmbx" id="metcmsbox" data-metcms_v="{$_M[config][metcms_v]}" data-patch="{$_M[config][met_patch]}">
	<div class="metcms_cont_left hidden-xs">
		<div class="">
			<a href="https://www.huawei.com/cn/?ic_medium=direct&ic_source=surlent" hidefocus="true">
				<img
					src="img/17f2dd9756d9d333bee8e60ce8c03e4c_222_222.jpg"
					alt=""
					title="东关村首页"
				/>
			</a>
		</div>
		<dl class="jslist fixedbar">
		<div class="fixedbar-box">
		<a target="_blank" href="{$_M['url']['site']}index.php?lang={$_M['lang']}" title="{$_M[word][homepage]}">{$_M[word][homepage]}</a>
		<span class="left">|</span><a href="" title="{$_M[word][backstage]}">{$_M[word][backstage]}</a>
<!--
EOT;
...
-->
		...
		<a href="{$_M['url']['adminurl']}n=ui_set&pageset=1" title="{$_M[word][visualization]}">{$_M[word][visualization]}</a>
<!--
EOT;
	}
echo <<<EOT
...
<!--
EOT;
if($_M['form']['switchurl']){
	$ifurl = urldecode($_M['form']['switchurl']);
}else{
