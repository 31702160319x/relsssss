...
class download_handle extends product_handle{
  ...
  public function one_para_handle($content) {
    ...
    $content['downloadurl'] = load::sys_class('handle', 'new')->url_transform($content['downloadurl']);
    if($content['downloadaccess']){
      ...
      $content['downloadurl'] = load::sys_class('handle', 'new')->url_transform("{$_M['url']['entrance']}?m=include&c=access&a=dodown&url={$url}&groupid={$groupid}");
    }
    return $content;
  }
