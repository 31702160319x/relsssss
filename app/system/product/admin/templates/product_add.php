...
<form method="POST" class="ui-from product_add" name="myform" action="{$_M[url][own_form]}a={$a}" target="_self">
	...
	<input type="hidden" name="class1_select" value="{$list[class1]}">
	<input type="hidden" name="class2_select" value="{$list[class2]}">
	<input type="hidden" name="class3_select" value="{$list[class3]}">
			...
			<dl>
				...
				<dd>
					<div class="fbox pull-left">
						...
						<span class="tips" style="display:block; margin-top:5px;">{$_M[word][tips12_v6]}</span>
					</div>
<!--
EOT;
...
-->
			...
			<dl>
				<dt><em class="required">*</em>{$_M[word][titletips]}</dt>
		...
		<div{$hide}>
			...
			<dl>
				<dd>
				<a href="{$_M[url][adminurl]}anyid={$_M['form']['anyid']}&n=parameter&c=parameter_admin&a=doparaset&module=3&id={$list[id]}" target="_blank">{$_M[word][parmanage]}</a>
