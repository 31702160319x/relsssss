...
class product_op extends news_op {
  ...
  public function del_by_class($classnow) {
	...
	/*复制*/
	public function list_copy($classnow, $toclass1, $toclass2, $toclass3, $tolang, $paras){
		global $_M;
		...
		if($paras){
			foreach($ids as $idkey => $idval){
				foreach($paras as $pkey => $pval){
					load::mod_class('parameter/parameter_op', 'new')->copy_list($class123['class1']['module'], $idkey, $pkey, $idval, $pval, $tolang);
				}
			}
		}
