...
<form method="POST" class="ui-from product_add" name="myform" action="{$_M[url][own_form]}a={$a}" target="_self">
	...
	<div class="v52fmbx">
		<h3 class="v52fmbx_hr">{$_M[word][baceinfo]}</h3>
		<dl>
...
echo <<<EOT
-->
		<span class="tips pull-left" style="margin-left:20px;"><a href="{$_M[url][site_admin]}index.php?lang={$_M[lang]}#metnav_25" target="_blank">{$_M[word][columumanage]}</a></span>
			</dd>
<!--
EOT;
}
...
-->		
		...
		<h3 class="v52fmbx_hr">{$_M[word][unitytxt_15]}<span class="tips"></span></h3>
		<dl>
